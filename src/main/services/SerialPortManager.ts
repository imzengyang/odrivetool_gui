...
import type { ODriveDevice, SerialPortEvent } from '../../shared/types';

export class SerialPortManager extends EventEmitter {
  private port: SerialPort | null = null;
  private parser: ReadlineParser | null = null;
  private connectedDevice: ODriveDevice | null = null;
   ...
   * 扫描可用的串口设备
   */
  async scanDevices(): Promise<ODriveDevice[]> {
    try {
      const ports = await SerialPort.list();
      const odriveDevices: ODriveDevice[] = [];

      for (const portInfo of ports) {
        // 检查是否为 ODrive 设备（基于 VID/PID 或描述）
        if (this.isODriveDevice(portInfo)) {
          const device: ODriveDevice = {
            id: portInfo.path,
            port: portInfo.path,
   ...
   * 连接到指定端口
   */
  async connect(portPath: string): Promise<ODriveDevice> {
    try {
   ...
   * 检查是否为 ODrive 设备
   */
  private isODriveDevice(portInfo: any): boolean {
    // 检查常见的 ODrive 设备标识
    ...
    return (
      odriveIdentifiers.some(id => 
        manufacturer.includes(id.toLowerCase()) ||
   ...
   * 获取设备信息
   */
  private async getDeviceInfo(): Promise<{ firmwareVersion: string; serialNumber: string }> {
    try {
      // 发送获取固件版本的命令
      await this.write('odrv0.fw_version\n');
      
      // 等待响应
      const firmwareVersion = await new Promise<string>((resolve) => {
